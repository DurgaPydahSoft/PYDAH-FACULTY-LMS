...
const HodTaskManagementSection = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const url = editingTaskId
        ? `${API_BASE_URL}/hod/tasks/${editingTaskId}`
        : `${API_BASE_URL}/hod/tasks`;
  ...
  const handleEdit = (task) => {
    ...
    setTaskForm({
      ...
      assignedTo: {
        ...
        hods: [],
        departments: task.assignedTo?.departments || (branchCode ? [branchCode] : [])
      },
      recurrence: {
  ...
  const handleDelete = async (taskId) => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/hod/tasks/${taskId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            <form className="flex-1 overflow-y-auto px-6 py-4 space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-4">
                ...
                <div>
                  ...
                  >
                    {Array.from(new Set(employees.map((employee) => (employee.department || '').toLowerCase()).filter(Boolean)))
                      .sort()
                      .map((department) => (
              ...
              <div>
                ...
                <div className="border border-gray-300 rounded-lg h-48 overflow-y-auto">
